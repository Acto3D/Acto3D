...
extension ViewController{
    ...
    internal func swapChannels(){
        ...
        let alert = NSAlert()
        alert.messageText = "Swap channels"
        alert.informativeText = "Please select the channels to swap."
        ...
        segment_output.alignment = .center
        segment_output.sizeToFit()
        segment_output.selectedSegment = 1
        
        if let loadChannels = renderer.imageParams.textureLoadChannel{
            switch loadChannels {
            case 1:
                segment_input.setEnabled(false, forSegment: 1)
                segment_input.setEnabled(false, forSegment: 2)
                segment_input.setEnabled(false, forSegment: 3)
                segment_output.setEnabled(false, forSegment: 1)
                segment_output.setEnabled(false, forSegment: 2)
                segment_output.setEnabled(false, forSegment: 3)
        ...
        let accessoryWidth:CGFloat = 300
        let accessoryHeight:CGFloat = 250
        let accessory = FlippedView(frame: NSRect(x: 0, y: 0, width: accessoryWidth, height: accessoryHeight))
        
        
        accessory.addSubview(segment_input)
        accessory.addSubview(segment_output)
        
        segment_input.setFrameOrigin(NSPoint(x: (accessoryWidth - segment_input.frame.width) / 2.0, y: 0))
        
        segment_output.setFrameOrigin(NSPoint(x: (accessoryWidth - segment_output.frame.width) / 2.0, y: segment_input.frame.maxY + 10))
        ...
        let ch1:UInt8 = segment_input.selectedSegment.toUInt8()
        let ch2:UInt8 = segment_output.selectedSegment.toUInt8()
        
        let processor = ImageProcessor(device: renderer.device, cmdQueue: renderer.cmdQueue, lib: renderer.mtlLibrary)
        processor.swapChannels(inTexture: mainTexture, channel1: ch1, channel2: ch2)
